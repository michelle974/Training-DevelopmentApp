...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ── coach / practitioner ────────────────────────────────────────────────
    // PRACTITIONER may be a single object {email,pin,name} OR an array of them,
    // so more than one person can have coach-level access. Either form works.
    const practitionerRaw = JSON.parse(process.env.PRACTITIONER || '{}');
    const practitioners = Array.isArray(practitionerRaw) ? practitionerRaw : [practitionerRaw];
    const coach = practitioners.find(function (p) {
      return p && p.email && p.email.toLowerCase() === email && p.pin === pin;
    });
    if (coach) {
      res.status(200).json({
        success: true,

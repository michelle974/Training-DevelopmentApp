...
// The single workspace every RACE READY project lives in (Training & Development).
// Project creation on the free tier needs only a workspace — no team GID required.
const WORKSPACE_GID = process.env.ASANA_WORKSPACE_GID || '1215329842549223';
...
// Create a project using the workspace-only form (free-tier safe; no team GID).
// Returns the created project record (has .gid).
async function createProject(name) {
  const d = await asana('/projects', 'POST', { data: { name: name, workspace: WORKSPACE_GID } });
  return d.data;
}
...
module.exports = {
  asana, cors, preflight, readBody, WORKSPACE_GID,
  encodeRecord, decodeRecord,
  listProjectTasks, listSubtasks, getTask, createTask, updateTask, deleteTask,
  createSubtask, getSections, createSection, createProject, addTaskToSection,

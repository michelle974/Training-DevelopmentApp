...
//   Tag: AM
//   Moves: [ {name,cat,type,video,spec:{...}}, ... ]
//
  ...
  if (!day.rest && Array.isArray(day.sessions)) {
    for (let i = 0; i < day.sessions.length; i++) {
      ...
      await A.createSubtask(dayTask.gid, {
        ...
        notes: A.encodeRecord({ Tag: s.tag || '', Moves: s.moves || [] })
      });
    }
  }
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      if (!planProjectId && !q.metaProjectId) { res.status(400).json({ error: 'planProjectId required' }); return; }
      ...
      if (q.action === 'library') {
        const r = await readSingletonJSON(q.metaProjectId, LIB_TASK, 'Library', []);
        res.status(200).json({ success: true, gid: r.gid, library: r.value });
        return;
      }
      if (q.action === 'templates') {
        const r = await readSingletonJSON(q.metaProjectId, TMPL_TASK, 'Templates', []);
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'save_library') {
        const r = await writeSingletonJSON(metaProjectId, LIB_TASK, 'Library', body.library || []);
        res.status(200).json({ success: true, gid: r.gid });
        return;
      }
      if (action === 'save_templates') {
        const r = await writeSingletonJSON(metaProjectId, TMPL_TASK, 'Templates', body.templates || []);
        res.status(200).json({ success: true, gid: r.gid });
        return;
      }
